    ...
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          ...
          {can("order:write") && <ShippingPanel id={id} order={order} onChanged={load} />}
        </div>
      </div>
    </div>
  );
...
/**
 * Shiprocket panel: shows shipment status and lets an admin walk the manual
 * courier-selection flow (check couriers -> assign AWB -> schedule pickup),
 * plus a manual "sync tracking" as a defensive fallback for when the
 * Shiprocket webhook doesn't fire (mirrors the payment sync pattern).
 */
function ShippingPanel({ id, order, onChanged }: { id: string; order: any; onChanged: () => void | Promise<void> }) {
  const [couriers, setCouriers] = useState<api.ServiceableCourier[] | null>(null);
  ...
  async function loadCouriers() {
    await run("couriers", async () => {
      const res = await api.getShippingCouriers(id);
  ...
  async function assignCourier() {
    ...
    await run("assign", () => api.assignShippingCourier(id, selectedCourierId));
  }

  async function schedulePickup() {
    await run("pickup", () => api.scheduleShippingPickup(id));
  }

  async function syncTracking() {
    await run("sync", () => api.syncShippingTracking(id));
  }

  if (!hasShipment) {
    return (
      <Panel className="p-5">
        <p className="text-sm font-medium text-ink-900 mb-1">Shipping</p>
        <p className="text-xs text-ink-500">
          No Shiprocket shipment yet — this is created automatically when the order moves to Processing.
        </p>
  ...
  return (
    <Panel className="p-5">
      <p className="text-sm font-medium text-ink-900 mb-3">Shipping</p>

      <dl className="space-y-1 text-xs text-ink-500">
        <div className="flex justify-between">
          <dt>Shiprocket order</dt>
          <dd className="text-ink-900">{shipment.shiprocketOrderId}</dd>
        </div>
        {shipment.courierName && (
          <div className="flex justify-between">
            <dt>Courier</dt>
            <dd className="text-ink-900">{shipment.courierName}</dd>
          </div>
        )}
        {shipment.awbCode && (
          <div className="flex justify-between">
            <dt>AWB</dt>
            <dd className="text-ink-900">{shipment.awbCode}</dd>
          </div>
        )}
        {shipment.lastTrackingStatus && (
          <div className="flex justify-between">
            <dt>Last tracking status</dt>
            <dd className="text-ink-900">{shipment.lastTrackingStatus}</dd>
          </div>
        )}
      </dl>
      ...
      {!hasAwb && (
        <div className="mt-4 space-y-2">
          <Button size="sm" variant="secondary" onClick={loadCouriers} disabled={busy !== null}>
              ...
              >
                ...
                {couriers.map((c) => (
                  <option key={c.courierId} value={c.courierId}>
                    {c.courierName} — ₹{c.rate} — {c.estimatedDeliveryDays} days
                  </option>
                ))}
              </select>
              <Button size="sm" onClick={assignCourier} disabled={!selectedCourierId || busy !== null}>
                {busy === "assign" ? "Assigning…" : "Assign courier & generate AWB"}
              </Button>
            </div>
          )}
        </div>
      ...
      {hasAwb && !shipment.pickupScheduledAt && (
        <div className="mt-4">
          <Button size="sm" onClick={schedulePickup} disabled={busy !== null}>
      ...
      {hasAwb && (
        <div className="mt-4">
          <Button size="sm" variant="secondary" onClick={syncTracking} disabled={busy !== null}>
            {busy === "sync" ? "Syncing…" : "Sync tracking now"}
          </Button>
        </div>
      )}
