...
function CustomerDetailInner() {
  ...
  useEffect(() => {
    api.getCustomer(id).then((res) => {
      setCustomer(res.data.customer);
      // The backend returns `recentOrders` (capped at 20) and a separate `stats`
      // aggregate — not `orders` with counts embedded on the customer (BUG-26).
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Panel className="p-5">
          ...
          <div className="mt-4 pt-4 border-t border-line grid grid-cols-2 gap-3 text-center">
            <div>
              <p className="font-display text-xl text-ink-950">{stats.totalOrders}</p>
              <p className="text-xs text-ink-500">Orders</p>
            </div>
            <div>
              <p className="font-display text-xl text-ink-950">{formatINR(stats.totalSpent)}</p>
              <p className="text-xs text-ink-500">Lifetime value</p>
            </div>
          </div>
        </Panel>
