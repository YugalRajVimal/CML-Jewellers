...
export class ApiRequestError extends Error {
  code: string;
  status?: number;
  // Raw `error.details` from the backend. For validation failures this is
  // `{ issues: [{ path, message }] }` — see errorMessage() below.
  details?: unknown;
...
/**
 * Turns any thrown value into text for a form's error line. When the backend
 * sent per-field validation issues, they are listed (one per line) instead of
 * the generic "Validation failed" message.
 */
export function errorMessage(e: unknown, fallback: string): string {
  if (e instanceof ApiRequestError) {
    const issues = (e.details as { issues?: ValidationIssue[] } | undefined)?.issues;
    if (Array.isArray(issues) && issues.length > 0) {
      return issues
        .map((i) => (i.path ? `${i.path}: ${i.message ?? "Invalid value"}` : i.message ?? "Invalid value"))
        .join("\n");
    }
    return e.message;
  }
...
// Multipart upload variant of request(): deliberately does NOT set
// Content-Type so the browser can add the multipart boundary itself. Used
// only by http.upload below (Cloudinary image upload via the backend's
// /admin/media/upload endpoint, which expects a multipart "image" field).
...
export const http = {
  ...
  // POST /admin/media/upload?folder=... — multipart/form-data, field name "image".
  upload: <T>(path: string, formData: FormData, params?: Record<string, unknown>) =>
    requestMultipart<T>(`${ADMIN_BASE}${path}${toQuery(params)}`, formData),
};
