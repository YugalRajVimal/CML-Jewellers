...
//   }>("/dashboard");
// }
// Statuses that count as "still open" for the dashboard's Open orders stat —
// everything short of a terminal state (Delivered / Cancelled).
const OPEN_ORDER_STATUSES = new Set(["Pending", "Confirmed", "Processing", "Shipped", "ReturnRequested"]);

export async function getDashboard() {
  const [summaryRes, trendRes, lowStockRes, breakdownRes, recentOrdersRes] = await Promise.all([
    http.get<{
      range: { from: string; to: string };
      revenue: number;
      orderCount: number;
    ...
    http.get<{ trend: { date: string; revenue: number; orders: number }[] }>("/dashboard/revenue-trend"),
    http.get<{ items: InventoryRow[] }>("/dashboard/low-stock", { limit: 5 }),
    // BUG-26: was hardcoded to 0 — the backend has this endpoint, it was just never called.
    http.get<{ breakdown: { status: string; count: number }[] }>("/dashboard/order-status-breakdown"),
    // BUG-26: was hardcoded to [] — reuse the existing orders list endpoint instead of
    // /dashboard/recent-activity, which returns the AuditLog feed, not orders.
    listOrders({ limit: 5 }).catch(() => ({ data: [] as unknown })),
  ]);
  ...
  const openOrders = breakdownRes.data.breakdown
    .filter((b) => OPEN_ORDER_STATUSES.has(b.status))
    .reduce((sum, b) => sum + b.count, 0);

  // The list endpoint's response shape varies by caller (see orders/page.tsx) — normalize the same way here.
  const rawRecent = recentOrdersRes.data as unknown;
  const recentOrdersRaw: Order[] = Array.isArray(rawRecent)
    ? (rawRecent as Order[])
    : Array.isArray((rawRecent as { orders?: Order[] })?.orders)
      ? (rawRecent as { orders: Order[] }).orders
      : [];
  ...
  return {
    ...
    data: {
      ...
      recentOrders: recentOrdersRaw.map((o: any) => ({
        ...
        customerName: o.userId?.name ?? o.customerName,
        total: o.total,
        status: o.status,
      })),
      lowStockRows,
...
// Backend actually returns { customer, addresses, recentOrders, stats } — not the
// { customer, orders } this used to assume (BUG-26).
export async function getCustomer(id: string) {
  return http.get<{
    customer: Customer;
    addresses: unknown[];
...
// BUG-26: this used to be a separate `me()` calling a non-existent `/auth/me`
// (dead code, unused, always 404s). The one real caller (`lib/auth.tsx`)
// already uses `whoami()` below, which hits the real `/auth/whoami` route —
// removed the dead duplicate rather than fix a call site that isn't used.
export async function whoami(): Promise<{ user: AdminUser; role: Role }> {
  const res = await http.get<{
    admin: Record<string, unknown>;
    role: Record<string, unknown>;
