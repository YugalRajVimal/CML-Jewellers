...
export interface CashfreeRefundResult {
  cfRefundId: string;
  status: string; // e.g. "SUCCESS" | "PENDING" | "ONHOLD"
}

/**
 * Issues a refund against an existing Cashfree order. `refundId` must be a
 * stable, idempotent identifier (we use our Refund document's _id) so retries
 * of the same refund never double-refund on Cashfree's side.
 */
export async function createCashfreeRefund(
  ...
  try {
    const response = await cfClient.post(`/orders/${cfOrderId}/refunds`, {
      refund_amount: amount,
      refund_id: refundId,
      refund_note: note ?? 'Customer return',
    });

    return {
      cfRefundId: response.data.refund_id,
      status: response.data.refund_status,
    };
  } catch (error) {
    const axiosError = error as { response?: { data?: unknown; status?: number }; message: string };
    logger.error('Cashfree refund creation failed', {
      message: axiosError.message,
      status: axiosError.response?.status,
      data: axiosError.response?.data,
    });
    throw AppError.internal('Failed to initiate refund with payment provider', 'CASHFREE_REFUND_FAILED');
  }
}
